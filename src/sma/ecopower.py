...
# Public: this is the canonical source for these coefficients, also consumed by ecopower-tariffs.
AFNAMETARIEF_EUR_PER_KWH: dict[FluviusRegion, float] = {
...
INJECTION_EPEX_COEFF      = 0.00098   # × EPEX(€/MWh)
INJECTION_FIXED_EUR_KWH   = -0.015
CONSUMPTION_EPEX_COEFF    = 0.00102
CONSUMPTION_FIXED_EUR_KWH = 0.004

# Per-kWh consumption add-ons (€/kWh, residential, tier 1: 0..3,000 kWh).
GSC_EUR_KWH                  = 0.011
WKK_EUR_KWH                  = 0.00392
BIJDRAGE_ENERGIE_EUR_KWH     = 0.0019261
ACCIJNS_TIER1_EUR_KWH        = 0.04748
...
def injection_price_eur_kwh(epex_eur_mwh: float) -> float:
    ...
    """
    return INJECTION_EPEX_COEFF * epex_eur_mwh + INJECTION_FIXED_EUR_KWH
...
def consumption_price_eur_kwh(epex_eur_mwh: float, region: FluviusRegion) -> float:
    ...
    """
    return (
        CONSUMPTION_EPEX_COEFF * epex_eur_mwh
        + CONSUMPTION_FIXED_EUR_KWH
        + GSC_EUR_KWH
        + WKK_EUR_KWH
        + AFNAMETARIEF_EUR_PER_KWH[region]
...
def break_even_epex_eur_mwh() -> float:
    ...
    return -INJECTION_FIXED_EUR_KWH / INJECTION_EPEX_COEFF
